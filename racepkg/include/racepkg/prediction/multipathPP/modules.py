...
class Decoder(nn.Module):
    ...
    def forward(self, target_scatter_numbers, target_scatter_idx, final_embedding, batch_size):
        ...
        coordinates = res[:, :, :12 * 2].reshape(
            batch_size, self._config["n_trajectories"], 12, 2)
        assert torch.isfinite(coordinates).all()
        a = res[:, :, 12 * 2: 12 * 3].reshape(
            batch_size, self._config["n_trajectories"], 12, 1)
        assert torch.isfinite(a).all()
        b = res[:, :, 12 * 3: 12 * 4].reshape(
            batch_size, self._config["n_trajectories"], 12, 1)
        assert torch.isfinite(b).all()
        c = res[:, :, 12 * 4: 12 * 5].reshape(
            batch_size, self._config["n_trajectories"], 12, 1)            
        assert torch.isfinite(c).all()


        epsi_vel = res[:, :, 12 * 5: 12 * 7].reshape(
            batch_size, self._config["n_trajectories"], 12, 2)            
        aa = res[:, :, 12 * 7: 12 * 8].reshape(
            batch_size, self._config["n_trajectories"], 12, 1)
        assert torch.isfinite(a).all()
        bb = res[:, :, 12 * 8: 12 * 9].reshape(
            batch_size, self._config["n_trajectories"], 12, 1)
        assert torch.isfinite(b).all()
        cc = res[:, :, 12 * 9: 12 * 10].reshape(
            batch_size, self._config["n_trajectories"], 12, 1)            
        assert torch.isfinite(c).all()       
        ...
        # probas = res[:, :, -1]
        probas = torch.ones(res[:, :, -1].shape).cuda()
        ...
        if self._config["trainable_cov"]:
            ...
            epsi_val_covariance_matrices = (torch.cat([
                torch.exp(aa) * torch.cosh(bb), torch.sinh(bb),
                torch.sinh(bb), torch.exp(-aa) * torch.cosh(bb)
            ], axis=-1) * torch.exp(cc)).reshape(
                epsi_vel.shape[0], epsi_vel.shape[1], epsi_vel.shape[2], 2, 2)
        else:
            _zeros, _ones = torch.zeros_like(a), torch.ones_like(a)
            covariance_matrices = torch.cat([_ones, _zeros, _zeros, _ones], axis=-1).reshape(
                coordinates.shape[0], coordinates.shape[1], coordinates.shape[2], 2, 2)
        return probas, coordinates, covariance_matrices, epsi_vel, epsi_val_covariance_matrices
...
class DecoderHandler(nn.Module):
    ...
    def forward(self, target_scatter_numbers, target_scatter_idx, final_embedding, batch_size):
        ...
        if self._return_embedding:
            ...
            for coeff, decoder in zip(random_head_selector, self._decoders):
                probas, coordinates, covariance_matrices, epsi_vel, epsi_vel_cov = decoder(
                    target_scatter_numbers, target_scatter_idx, final_embedding, batch_size)
                return (probas, coordinates, covariance_matrices, epsi_vel, epsi_vel_cov,  max(self._n_decoders / random_head_selector.sum(), 1))
